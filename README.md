...
Docker와 Docker Compose를 이용해 **3-tier 구조**로 구현한 웹 서비스입니다.
버튼을 누르면 1~45 중 **6개 번호를 자동 추첨**하고, 결과를 DB에 저장하며,
...
이 프로젝트는 역할이 분리된 3개의 컨테이너로 구성됩니다.
...
> 참고: 비밀번호는 과제 시연 편의를 위해 `docker-compose.yml`에 평문으로 두었습니다.
> 실제 배포라면 `.env` 파일이나 시크릿으로 분리하는 것이 좋습니다.
...
- [ ] `docker compose up --build` 로 컨테이너 3개가 모두 실행됨
- [ ] `http://localhost:8080` 화면이 열림
- [ ] "번호 생성하기" 클릭 시 공 6개가 표시됨
- [ ] 추첨 기록 목록에 회차가 누적됨
- [ ] `docker compose down` 후 다시 올려도 기록이 유지됨 (볼륨)
